# BIO Release Note For Coordination (Wave-1)
...
1. Bio consumes IMC family contract artifacts by reference only; no runtime coupling to IMC package internals.
2. Canonical cross-family metric authority is pinned to IMC compatibility vector (`total_words=844`).
...
1. Platform owner: mark required checks as:
